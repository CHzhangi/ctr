...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask):
        """
        input_Q: [batch_size, len_q, d_model]     X
        input_K: [batch_size, len_k, d_model]     X
        input_V: [batch_size, len_v(=len_k), d_model]    X
        attn_mask: [batch_size, seq_len, seq_len]
        """
        residual, batch_size = input_Q, input_Q.size(0)
        # 下面的多头的参数矩阵是放在一起做线性变换的，然后再拆成多个头，这是工程实现的技巧
        # B: batch_size, S:seq_len, D: dim
        # 其实就是X* W_Q= Q  x的形状是seq_len,d_model,经过矩阵W_Q(相当于一个全连接层)，变成了Q（seq_len,d_k(未知，主要是在这儿无所谓)）
        #然后呢由于是多头，我们会使用不同个WQ WK WV生成多个Q K V那其实我们只要全连接层，把他变成横向的多个。也就是把权重矩阵变成（seq_len,d_k*n_heads），然后再分割。
        ...
        # 因为是多头，所以mask矩阵要扩充成4维的（和Q*KT 同维度）
...
class Encoder(nn.Module):
    ...
    def forward(self, enc_inputs):
        ...
        # Encoder输入序列的pad mask矩阵,主要是因为序列长度可能不同，有的序列可能短，然后将序列长度补齐
        # 对序列的每个位置的元素进行编码
